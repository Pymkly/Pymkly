...
@tool
def add_attendee(event_id: str, emails: list= None) -> str:
    """Ajoute un invité à un événement Google Calendar. Params: event_id (ID de l'événement), emails (emails des invités)."""
    try:
        if emails:
            # Valider l'email
            for email in emails:
...
@tool
def remove_attendee(event_id: str, emails: list = None) -> str:
    """Retire un invité d'un événement Google Calendar. Params: event_id (ID de l'événement), emails (emails des invités)."""
    try:
        if emails :
            # Valider l'email
            for email in emails:
...
def delete_calendar_event(event_id: str) -> str:
    """Supprime un événement Google Calendar. Param: event_id (ID de l'événement)."""
    try:
        service = get_calendar_service()
...
@tool
def shift_calendar_event(event_id: str, new_start_time: str, new_end_time: str) -> str:
    """Déplace un événement Google Calendar à de nouvelles dates/heures. Params: event_id (ID de l'événement), new_start_time/new_end_time (ISO format ex: '2025-10-01T14:00:00+03:00')."""
    try:
...
def list_calendar_events(start_date: str, end_date: str) -> str:
    """Liste les événements Google Calendar entre start_date et end_date (ISO format ex: '2025-10-01T00:00:00+03:00'). . Retourne l'ID, le titre, la date et les invités de chaque événement."""
    try:
        # Valider le format ISO
...
@tool
def create_calendar_event(summary: str, start_time: str, end_time: str, description: str = "", attendees: list = None) -> str:
    """Crée un événement Google Calendar. Params: summary (titre), start_time/end_time (ISO format ex: '2025-10-01T14:00:00'), description, attendees (liste d'emails à inviter pendant l'evenement, optionnel)."""
    try:
        service = get_calendar_service()
        event = {
            'summary': summary,
            'description': description,
            'start': {'dateTime': start_time, 'timeZone': 'Europe/Paris'},  # Ajuste le timezone
